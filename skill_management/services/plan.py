...
class PlanService:
    ...
    async def create_or_update_plan_by_admin(self, plan_request: PlanCreateAdminRequest) -> PlanCreateResponse:
        if plan_request.plan_id is not None:
            """
            It is an update operation
            """
    ...
    async def _update_plan_by_admin(self, plan_request: PlanCreateAdminRequest):
        profile_crud_manager = ProfileRepository()
        old_profile = await profile_crud_manager.get_by_query(
            query={
    ...
    async def _create_plan_by_admin(self, plan_request: PlanCreateAdminRequest):
        profile_crud_manager = ProfileRepository()
        profile_plans = await profile_crud_manager.get_by_query(
            query={
                "_id": plan_request.profile_id
