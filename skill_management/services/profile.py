...
from skill_management.enums import FileTypeEnum, DesignationStatusEnum
...
from skill_management.schemas.designation import ProfileDesignation, ProfileDesignationResponse, DesignationDataCreate
...
from skill_management.schemas.profile import ProfileBasicForAdminRequest, ProfilePersonalDetails, ProfileResponse, \
    ProfilePersonalDetailsResponse, ProfileUpdateByAdmin
...
class ProfileService:
    # pass
    @staticmethod
    async def create_or_update_user_profile_by_admin(profile: ProfileBasicForAdminRequest) -> ProfileResponse:
        ...
        if profile.profile_id is None:
            ...
            designation: Designations | None = await Designations.get(profile.designation_id)
            if designation is None:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                    detail="Invalid designation of the profile")

            gender_data = await Gender.get(profile.gender)
            if gender_data is None:
                raise

            profile_status_object: ProfileStatus = await ProfileStatus.get(profile.profile_status.value)
            ...
            designation_status_object: DesignationStatus = await DesignationStatus.get(profile.designation_status.value)
            ...
            # new_designation: DesignationDataCreate | None = None
            # if profile.designation_id is not None:
            #     designation: Designations | None = await Designations.get(profile.designation_id)  # type: ignore
            #     if designation is None:
            #         raise HTTPException(status_code=status.HTTP_400_BADREQUEST,
            #                             detail="Ypu must provide a valid designation")
            # new_designation = DesignationDataCreate(designation=designation.designation,
            #                                         designation_id=designation.id,
            #                                         start_date=None, end_date=None)
            # all_designations.append(new_designation)

            update_item = ProfileUpdateByAdmin(name=profile.name,
                                               date_of_birth=profile.date_of_birth,
                                               gender=profile.gender,
                                               mobile=profile.mobile,
                                               ...
                                               designation_status=profile.designation_status,
                                               about=profile.about)
            item_dict = update_item.dict(exclude_unset=True, exclude_none=True)
            if profile.designation_id is None:
                db_profile: Profiles = await profile_crud_manager.update(id_=profile.profile_id,  # type: ignore
                                                                         item_dict={"personal_detail": item_dict})
            else:
                all_experience_id = [experience.experience_id for experience in old_profile.experiences]
            ...
            # profile_status_object: ProfileStatus = await ProfileStatus.get(  # type: ignore
            #     db_profile.profile_status.value)  # type: ignore
            # designation_status_object: DesignationStatus = await DesignationStatus.get(  # type: ignore
            #     db_profile.designation_status.value)  # type: ignore
            # designation_object: Designations = await Designations.get(db_profile.designation_id)  # type: ignore
            ...
            for skill_ in db_profile.skills:
                ...
                skill_status_object: Status = await Status.get(
                ...
                skill_list.append(
                    ProfileSkillResponse(
                        ...
                        status=ResponseEnumData(id=skill_status_object.id,
                                                name=(
                                                    await Status.get(
                                                        skill_.status.value  # type: ignore
                                                    )
